...
export default function CrmExecutiveAnalytics() {
  ...
  // ----------------------------------------------------
  // DASHBOARD 3: Lead Score Distribution Math
  // ----------------------------------------------------
  ...
  // ----------------------------------------------------
  // DASHBOARD 4: Service Scope Matrix Math
  // ----------------------------------------------------
  const knownServices = [
  ...
  // ----------------------------------------------------
  // DASHBOARD 5: Digital Conversion Rate & Profit Cash Flow Math
  // ----------------------------------------------------
  ...
  return (
    <div className="space-y-6 mb-8 max-w-7xl mx-auto">
      ...
      {/* DASHBOARDS GRID 1: Financial & Channel Intelligence */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* DASHBOARD 2: Channel Performance & Sales Velocity */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* DASHBOARDS GRID 2: Visual Distribution Matrices */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">

        {/* DASHBOARD 3: Lead Score Category Distribution (SVG Donut Ring Chart) */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className={`font-black text-base tracking-tight ${isLight ? "text-slate-950" : "text-zinc-50"}`}>
                  Lead Score Category Distribution
                </h3>
                <p className={`text-xs font-semibold ${isLight ? "text-slate-500" : "text-zinc-400"}`}>
                  Quality Tier Breakdown (Donut Chart)
                </p>
              </div>
            </div>
            <span className="text-xs font-black px-2.5 py-1 rounded-lg bg-rose-500/15 text-rose-700 dark:text-rose-400 border border-rose-500/20 tabular-nums">
              🔥 {hotCount} HOT ({hotPct}%)
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className={`p-2.5 rounded-xl border text-center ${isLight ? "bg-rose-50/60 border-rose-200/60" : "bg-rose-950/20 border-rose-800/40"}`}>
              <span className="text-[10px] font-bold text-rose-600 dark:text-rose-400 block">HOT Pipeline</span>
              <span className="text-xs font-black text-rose-700 dark:text-rose-400 tabular-nums">${hotValue.toLocaleString()}</span>
            </div>
            <div className={`p-2.5 rounded-xl border text-center ${isLight ? "bg-amber-50/60 border-amber-200/60" : "bg-amber-950/20 border-amber-800/40"}`}>
              <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 block">WARM Pipeline</span>
              <span className="text-xs font-black text-amber-700 dark:text-amber-400 tabular-nums">${warmValue.toLocaleString()}</span>
            </div>
            <div className={`p-2.5 rounded-xl border text-center ${isLight ? "bg-sky-50/60 border-sky-200/60" : "bg-sky-950/20 border-sky-800/40"}`}>
              <span className="text-[10px] font-bold text-sky-600 dark:text-sky-400 block">COLD Pipeline</span>
              <span className="text-xs font-black text-sky-700 dark:text-sky-400 tabular-nums">${coldValue.toLocaleString()}</span>
            </div>
          </div>
        </motion.div>

        {/* DASHBOARD 4: Service Scope Matrix (Vertical Column Chart) */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="grid grid-cols-5 gap-2 sm:gap-3 items-end h-44 pt-4 pb-2 px-1 border-b border-slate-100 dark:border-zinc-800/80 mb-3">
            {knownServices.map((serviceName) => {
              ...
              const heightPct = maxServiceValue > 0 ? Math.max(14, Math.round((data.value / maxServiceValue) * 100)) : 14;
              ...
              return (
                <div key={serviceName} className="flex flex-col items-center h-full justify-end group">
                  <span className={`text-[10px] font-black tabular-nums mb-1 transition-opacity ${
                  ...
                  <div className="w-full flex-1 flex items-end justify-center">
                    <motion.div
                      ...
                      transition={{ duration: 0.6, ease: "easeOut" }}
                      className={`w-full max-w-[36px] rounded-t-xl ${style.bar} shadow-sm group-hover:brightness-110 transition-all cursor-pointer`}
                  ...
                  <div className="text-center mt-2 w-full">
                    ...
                    <span className={`text-[9px] font-bold px-1.5 py-0.2 rounded-md ${style.bg} ${style.text} inline-block mt-0.5`}>
                      {data.count} deals
                    </span>
                  </div>
                </div>
      ...
      {/* DASHBOARD 5 (NEW): Digital Conversion Rate & Profit Cash Flow Matrix */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className={`text-xs font-semibold mt-0.5 ${isLight ? "text-slate-600" : "text-zinc-400"}`}>
                Mô hình hóa Tỷ lệ Chuyển đổi Lead Kỹ thuật số (Digital Conversion) và Dòng tiền Lợi nhuận ròng thực thu.
              </p>
            </div>
          </div>
